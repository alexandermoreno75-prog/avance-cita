<section class="page-header"><div><h1>Registrar paciente</h1><p>Complete los datos
obligatorios.</p></div></section>
<form class="panel form-grid" method="post" action="<?= e(url('/patients')) ?>" novalidate>
 ...
 <option value="<?= $type ?>" <?= ($data['document_type'] ?? 'CC') === $type ? 'selected' : '' ?>><?= $type
?></option>
 <?php endforeach; ?>
 </select>
 <?php if (isset($errors['document_type'])): ?><small class="field-error"><?= e($errors['document_type'])
?></small><?php endif; ?>
 </div>
 <div>
 <label for="document_number">Número de documento</label>
 <input id="document_number" name="document_number" maxlength="30" value="<?=
e($data['document_number'] ?? '') ?>" required>
 <?php if (isset($errors['document_number'])): ?><small class="field-error"><?= e($errors['document_number']) ?
></small><?php endif; ?>
 </div>
 <div>
 ...
 <?php if (isset($errors['last_name'])): ?><small class="field-error"><?= e($errors['last_name']) ?></small><?php
endif; ?>
 </div>
 <div>
 <label for="birth_date">Fecha de nacimiento</label>
 <input id="birth_date" name="birth_date" type="date" max="<?= e(date('Y-m-d')) ?>" value="<?=
e($data['birth_date'] ?? '') ?>" required>
 <?php if (isset($errors['birth_date'])): ?><small class="field-error"><?= e($errors['birth_date']) ?></small><?php
endif; ?>
 </div>
 <div>
